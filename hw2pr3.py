...
def linreg(X, y, reg=0.0):
	...
	"*** YOUR CODE HERE ***"
	num_cols = X.shape[1]
	reg_mat = np.eye(num_cols) * reg
	reg_mat[0, 0] = 0
	W_opt = np.matmul(np.matmul(np.linalg.inv(reg_mat + np.matmul(X.T, X)), X.T), y)
...
def find_RMSE(W, X, y):
	...
	"*** END YOUR CODE HERE ***"
	return np.linalg.norm(predict(W, X) - y) / np.sqrt(len(y))
...
def RMSE_vs_lambda(X_train, y_train, X_val, y_val):
	...
	RMSE_list = []
	reg_list = np.sort(np.random.uniform(0.0, 150.0, 150))
	W_list = []
	"*** YOUR CODE HERE ***"
	for reg in reg_list:
...
def grad_descent(X_train, y_train, X_val, y_val, reg=0.0, lr_W=2.5e-12, \
	...
	print('xw', X_train.shape, W.shape)
	
	def compute_grads():
		# residuals = X_train @ W - b * ones - y_train
		# print('resid: ', residuals.shape, (X_train @ W).shape, (b * ones).shape, y_train.shape)
	...
	print('dims: ', m_train, W_grad.shape)
	...
	#	3) Upgrade W and b
	#	4) Keep iterating whie the number of iterations is less than the
	...
	while np.linalg.norm(W_grad) > eps and np.linalg.norm(b_grad) > eps \
		...
		W = W - lr_W * W_grad
		b = b - lr_b * b_grad
		# print('new W', W.shape, b)
		W_grad, b_grad = compute_grads()
		train_rmse = np.linalg.norm(X_train @ W + b - y_train)/np.sqrt(m_train)
		val_rmse = np.linalg.norm(X_val @ W + b - y_val)/np.sqrt(m_val)
